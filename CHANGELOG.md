...
<!-- ko -->
### 수정
- **설정 로드 오류 수정** — 이전 최적화 과정에서 누락되었던 언어 및 트레이 표시 기준 설정의 초기 로딩 로직을 복구했습니다. 이제 설정창을 열 때 현재 값이 드롭다운에 올바르게 표시됩니다.
- **버전 번호 동기화** — v1.21.8 배포 시 누락되었던 `csproj` 내 버전 번호를 정상화했습니다.
<!-- /ko -->
...
<!-- ko -->
### 수정
- **다국어 지원 보완** — 팝업 내 "Extra Credits" 문구가 하드코딩되어 있던 문제를 해결하고 다국어(한국어, 중국어, 일본어, 영어) 지원 추가
<!-- /ko -->

<!-- en -->
### Fixed
- **Improved localization** — Resolved an issue where "Extra Credits" was hardcoded in the popup; added support for multiple languages (ko, zh, ja, en)
<!-- /en -->
...
<!-- ko -->
### 수정
- **Codex/Gemini 사용량 바 미표시 버그 수정** — WPF ProgressBar 인라인 템플릿에 `PART_Track` 누락으로 바가 채워지지 않던 문제 해결
- **새로고침 버튼 미동작 수정** — `MouseLeftButtonDown` 이벤트가 버튼 클릭을 가로막던 문제 수정 (`e.Handled` 체크 추가)
- **버전 버튼 클릭 불가 수정** — `UpdateCheckLabel`이 빈 문자열일 때 버튼이 사실상 투명하여 클릭이 불가했던 문제 수정

### 개선
- **앱 제목 일반화** — 공급자 선택에 따라 바뀌던 타이틀을 "AI 에이전트 사용량"으로 고정
- **트레이 툴팁 일반화** — "Gemini 사용량"으로 고정되던 툴팁을 3개 공급자 요약으로 개선
- **설정창 단순화** — 불필요한 "차트 표시 기준" 드롭다운 제거
- **레이아웃 재구성** — 히스토리 차트·오늘의 토큰을 Claude 섹션 내부로 이동 (클로드 기준 데이터이므로)
- **오늘의 토큰 색상 구분** — 입력/출력/캐시읽기/캐시쓰기 토큰을 4가지 색으로 구분 표시
- **불필요한 ⤓ 버튼 제거** — 설정 옆에 의도치 않게 표시되던 내보내기 버튼 제거
- **섹션 밀도 복원** — 이전 버전 대비 좁아졌던 패딩·간격 원복
<!-- /ko -->
...
<!-- ko -->
### 추가
- **Gemini CLI 실시간 사용량 파싱** — `.gemini/tmp` 로그 파일을 분석하여 토큰 기반 사용량 %를 계산하도록 개선
- **트레이 게이지 기준 명시** — 현재 트레이 아이콘 게이지가 어떤 공급자를 기준으로 하는지 툴팁과 앱 메인 상태 표시줄에 명시

### 개선
- **추가 사용량(Extra Usage) 위치 조정** — Claude 섹션 내부(7일 윈도우 아래)로 이동하여 맥락 통합
- **메인 팝업 레이아웃 최적화** — 3개 공급자 동시 노출에 최적화된 더 콤팩트한 디자인 적용
<!-- /ko -->
...
- **공급자별 알림 지원** — Codex와 Gemini의 사용량 임계값 도달 시에도 알림을 받을 수 있도록 확장

### 개선
- **트레이 아이콘 및 툴팁 통합** — 트레이 툴팁에서 3개 공급자의 사용량 요약을 한눈에 확인 가능 (예: `Claude: 80% | Codex: 20% | Gemini: 10%`)
- **트레이 메뉴 확장** — 우클릭 메뉴에서 모든 공급자의 상세 상태(5h/7d 사용량 포함)를 즉시 확인 가능
- **메인 팝업 레이아웃 최적화** — 3개 공급자 표시에 맞춰 수직 공간을 최적화(ProgressBar 4px 축소, 폰트 조정)하여 콤팩트한 레이아웃 제공
- **차트 제목 동적 바인딩** — 하단 히스토리 차트가 현재 어떤 공급자의 데이터인지 명확히 표시
...
### 수정
- **업데이트 UX 개선** — 별도 Updater.exe로 업데이트 진행 상황을 표시 (진행률 창 표시)
...
### 수정
- **릴리즈 파일 정리** — SHA256 검증 파일을 SHA256.txt 하나로 통일 (구 ClaudeUsageTray.sha256 제거)
- **자동 업데이트 안정성 개선** — 업데이트 시 기존 프로세스가 완전히 종료될 때까지 대기 후 exe 파일 복사 (이전 버전에서 계속 유효)
...
<!-- ko -->
### 수정
- **자동 업데이트 안정성 개선** — 업데이트 시 기존 프로세스가 완전히 종료될 때까지 대기 후 exe 파일 복사 (race condition 해결)
<!-- /ko -->

<!-- en -->
### Fixed
- **Auto-update stability** — Wait for old process to fully terminate before copying exe file (fixes race condition)
<!-- /en -->

<!-- zh -->
### 修复
- **自动更新稳定性** — 在复制exe文件之前等待旧进程完全终止（解决竞争条件）
<!-- /zh -->

<!-- ja -->
### 修正
- **自動更新の安定性** — exeファイルをコピーする前に古いプロセスが完全に終了するのを待つ（競合状態の解決）
<!-- /ja -->
...
<!-- ko -->
### 추가
- **Tray 메뉴 다음 갱신 시간 표시** — 우클릭 메뉴에 다음 자동 갱신까지 남은 시간 표시

### 이슈 닫기
- #50: 종료 메뉴에 '다음 갱신까지 남은 시간' 표시
- #51: 설정 창에서 갱신 주기 입력 가이드 표시 (기존 구현)
- #48: Tray 우클릭 메뉴에 현재 사용량 표시 (기존 구현)
<!-- /ko -->

<!-- en -->
### Added
- **Tray menu next refresh time** — Shows time until next auto-refresh in right-click menu

### Closed Issues
- #50: Next refresh time in quit menu (already implemented)
- #51: Refresh interval hint (already implemented)
- #48: Current usage in tray menu (already implemented)
<!-- /en -->

<!-- zh -->
### 新增
- **托盘菜单下次刷新时间** — 在右键菜单显示距离下次自动刷新的剩余时间
...
<!-- ja -->
### 追加
- **Trayメニュー次回更新時間** — 右クリックメニューに次回自動更新までの残り時間を表示

### クローズした 이슈
- #50: 終了メニューに次回更新時間を表示（既に実装済み）
- #51: 更新間隔の入力ガイドを表示（既に実装済み）
- #48: Tray右クリックメニューに使用量を表示（既に実装済み）
<!-- /ja -->
