        ...
        while ((line = reader.ReadLine()) != null)
        {
            ...
            try
            {
                ...
                if (root.TryGetProperty("timestamp", out var tsEl))
                {
                    ...
                    {
                        if (ts.ToUniversalTime().Date < sinceDate) continue;
                        fileHadActivity = true;
                        if (ts.ToUniversalTime() > stats.LastActivity)
                            stats.LastActivity = ts.ToUniversalTime();
                    }
                }
                ...
                if (usageEl.TryGetProperty("input_tokens", out var inp))
                    stats.TotalInputTokens += inp.GetInt64();
                if (usageEl.TryGetProperty("output_tokens", out var out_))
                    stats.TotalOutputTokens += out_.GetInt64();
                if (usageEl.TryGetProperty("cache_read_input_tokens", out var cr))
                    stats.TotalCacheReadTokens += cr.GetInt64();
                if (usageEl.TryGetProperty("cache_creation_input_tokens", out var cw))
                    stats.TotalCacheWriteTokens += cw.GetInt64();
