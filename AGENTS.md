...
- **v1.21.5**: 사용량 화면 가시성 제어(데이터 없는 공급자 숨기기), 동적 트레이 표시 기준(최근 7일 최대치 기반), 클로드 사용량 부재 시 UI 단순화 및 통계 로직 강화.
- **v1.21.4**: 설정 창 컨트롤 다크 테마 통일(ScrollBar, CheckBox 등), 일관된 시각적 피드백 제공.
