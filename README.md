...
## 이슈
- `개선` 멀티 PC 사용량 스냅샷 동기화 구현 계획 (2026-07-09): 같은 계정을 데스크탑/노트북에서 사용할 때 API 실패 또는 로컬 로그 차이로 표시가 어긋나는 문제를 줄이기 위해, 자격증명과 원본 로그는 동기화하지 않고 PC별 표시용 스냅샷만 공유하는 구현 계획을 정리. #78
