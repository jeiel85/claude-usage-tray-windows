...
| # | 제목 | 분류 | 상태 |
|---|------|------|------|
| [#59] | Claude/Codex/Gemini 게이지 문구 개선(사용량·잔량·초기화 표시 통일) 및 Gemini 게이지 미변경 원인 수정 | 개선 | 완료 |
...
| # | 제목 | 분류 | 상태 |
|---|------|------|------|
| [#58_old] | 사용량 팝업 가독성 개선 (폰트 크기 및 시인성 강화) | UI | 완료 |
| [#59] | 프로젝트 및 README 버전 동기화 (v1.17.2) | 관리 | 완료 |

### v1.17.1 (완료)
| # | 제목 | 분류 | 상태 |
|---|------|------|------|
| [#56] | 팝업 보조 텍스트 폰트 크기 원복 | 개선 | 완료 |
| [#57_old] | 설정창 초기 로딩 중 저장 이벤트 가드 추가 | 개선 | 완료 |
| [#55] | Gemini CLI 실시간 토큰 합산 및 % 계산 구현 | 수정 | 완료 |
| [#54] | 트레이 아이콘 게이지 기준 공급자 명시 (툴팁/상태바) | 개선 | 완료 |
| [#53] | 추가 사용량(Extra Usage) 위치를 Claude 섹션으로 통합 | 개선 | 완료 |

### v1.17.0
| # | 제목 | 분류 | 상태 |
|---|------|------|------|
| [#41] | 설정 창 단축키 (ESC, Alt+F4, Ctrl+W) 지원 | 개선 | 완료 |
